...
class TestFormatModuleName:
    ...
    def test_should_remove_folders_from_the_name(self, patterns):
        formatted = formatters.format_module_name(
            'tests/sub/test_module.py', patterns
        )
...
class TestTrimMultiLineText:
    ...
    def test_should_srip_spaces_from_multiple_lines(self):
        assert (
            formatters.trim_multi_line_text(
                '''
...
class TestFormatResult:
    def test_should_not_pad_single_line_text(self):
        assert formatters.format_result_str('>>> ', 'some text') == (
            '>>> some text'
        )
    ...
    def test_should_pad_the_following_lines_to_the_width_of_given_characters(
        ...
        text = (
            'first line{0}' 'second line{0}' 'third line{0}' 'fourth line'
        ).format(os.linesep)
        assert formatters.format_result_str('>>> ', text) == (
            '>>> first line{0}'
            '    second line{0}'
            '    third line{0}'
            '    fourth line'.format(os.linesep)
        )

    def test_should_remove_empty_lines(self):
        text = (
            'first line{0}' '{0}' 'second line{0}' '{0}' '{0}' 'third line'
        ).format(os.linesep)
        assert formatters.format_result_str('>>> ', text) == (
            '>>> first line{0}'
            '    second line{0}'
            '    third line'.format(os.linesep)
        )
...
class TestIncludeParametrized:
    def test_should_return_title_when_no_parameters_are_found(self):
        assert (
            formatters.include_parametrized(
